...
class Bank{
    customer:Customer[]=[];
    account:BankAccount[]=[];
    customerAdd(obj:Customer){
        this.customer.push(obj); 
    }
    accountNoAdd(obj:BankAccount){
        this.account.push(obj);
    }
    updateTransaction(accountobj:BankAccount){
        let updateAcc=this.account.filter(
            (acc) => acc.accountNo !== accountobj.accountNo
        );
...
async function bankService(bank:Bank){
    do{
        ...
        if (services.select == chalk.bold.green("view Balance")){
          ...
          let account = myBANK.account.find(
            (acc) => acc.accountNo == response.AccNo
          );
          if (!account){
